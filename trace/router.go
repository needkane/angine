...
func NewRouter(logger *zap.Logger, config *viper.Viper, state *sm.State, priv *types.PrivValidator) *Router {
...
// SetPrivValidator
func (rt *Router) SetPrivValidator(priv *types.PrivValidator) {
...
func (rt *Router) request(requestHash []byte, fromPeer []byte, data []byte, onlyValidator bool) (broadcast bool, err error) {
	...
	rt.reactor.Switch.Peers().Get(fmt.Sprintf("%X", fromPeer)).Send(SpecialOPChannel, struct{ Message }{resp})
	return true, nil
}
